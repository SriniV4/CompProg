...
void solve(){
    ...
    reverse(all(hull));
    // for(auto& d :hull)cout << d << "\n";
    cout << fixed << setprecision(12);
    for(int i =1 ;i<sz(hull);i++){
        ll diff = hull[i].y - hull[i-1].y;
        ll dx = hull[i].x - hull[i-1].x;
        for(int j =hull[i-1].x;j<hull[i].x;j++)if(j){
            ll ans = diff * (j - hull[i-1].x);
ll q = ans / dx, r = ans % dx;
if (r < 0) { q--; r += dx; }          // convert trunc to floor
cout << q*MAXN + (r*MAXN)/dx + hull[i-1].y*MAXN << "\n";
            // cout << j << " ";
        }
    }
}
